...
function readPrepublishPreviews() {
  return storageCache.prepublishPreviews || [];
}

async function writePrepublishPreviews(items) {
  storageCache.prepublishPreviews = items;
  writeJsonBackup(PREPUBLISH_PREVIEWS_FILE, items);
  await persistPrepublishPreviews(items);
  return items;
}
...
const storageCache = {
  ...
  prepublishPreviews: readJsonArray(PREPUBLISH_PREVIEWS_FILE),
...
async function ensurePrepublishPreviewsTable() {
  if (!pgPool) return;
  await pgPool.query(`
    create table if not exists prepublish_previews (
      id text primary key,
...
async function persistPrepublishPreviews(items) {
  if (!pgPool) return;
  try {
    await ensurePrepublishPreviewsTable();
    for (const item of items) {
      await pgPool.query(
        ...
        [
          ...
          Number(item.readiness_score || 0),
          item.expected_reaction || "",
          JSON.stringify(Array.isArray(item.risk_warnings_json) ? item.risk_warnings_json : []),
          JSON.stringify(Array.isArray(item.checklist_json) ? item.checklist_json : []),
          pgColumnDate(item.created_at)
        ]
      );
    }
...
function narrativeFlexibleBeats({ profile, diversity, hero, relation, object, setting, twistDevice, peakEmotion, seed }) {
  const truth = diversity.turn || profile.turn || "the old accusation had protected the wrong person";
  const conflict = diversity.conflict || profile.conflict || "a family betrayal comes into the open";
  const pressure = pick([
    `${hero} had expected excuses, maybe anger, maybe the usual tired speech about family peace. Instead, ${relation} asked everyone else to leave the room, and that frightened her more than shouting.`,
    `For years, ${conflict} had been explained with shrugs and half-sentences. Now ${object} made every shrug look planned.`,
    `${hero} noticed who reached for the door and who reached for the chair. Small movements tell the truth before people do.`,
    `Nobody in ${setting} seemed surprised enough. That was the detail that made ${hero} cold.`,
    `${relation} tried to make the moment smaller, but the silence around ${object} kept getting larger.`
  ], `${seed}:pressure`);
  const dialogueTurn = pick([
    `"I want the first version," ${hero} said. "Not the version you practiced."`,
    `"If you loved me, you should have trusted me with the ugly part," ${hero} said.`,
    `"Stop protecting the person who created the wound," ${hero} said.`,
    `"I am not asking who suffered," ${hero} said. "I am asking who lied."`,
    `"You do not get to call me dramatic after hiding this in my own house," ${hero} said.`
  ], `${seed}:dialogue-turn`);
  const evidence = pick([
    `${twistDevice} did not explain everything, but it moved the story out of rumor and into proof.`,
    `The important part was not the object itself. It was the date, the name, and the nervous correction written in the margin.`,
    `A smaller secret fell out first, almost by accident, and then the larger one followed because nobody could close the drawer fast enough.`,
    `The proof was ordinary enough to be missed by a stranger and devastating enough to split a family dinner in two.`,
    `${hero} read the line twice. The second time, she understood why everyone had been so careful with her anger.`
  ], `${seed}:evidence`);
  const reveal = pick([
    `Then the hidden part came out: ${truth}.`,
    `Only after the second silence did ${relation} admit the sentence everyone had been walking around: ${truth}.`,
    `The turn was not theatrical. It was worse because it sounded tired: ${truth}.`,
    `What finally broke the room was not a scream, but the plain explanation: ${truth}.`,
    `${relation} said it almost gently, and that made it harder to bear: ${truth}.`
  ], `${seed}:reveal`);
  const escalation = pick([
    `${hero} felt ${peakEmotion}, then shame for having trusted the wrong memory, then anger again because shame was not hers to carry.`,
    `The old version of the family did not collapse all at once. It lost one beam, then another, until ${hero} could see daylight through it.`,
    `Someone began to cry, but ${hero} could not comfort them. Comfort had been used too many times to delay the truth.`,
    `The room waited for her to become either cruel or forgiving. She chose neither. She chose to keep asking.`,
    `It was the first honest conversation they had ever had, and nobody knew where to put their hands.`
  ], `${seed}:escalation`);
  const choice = pick([
    `${hero} did not decide the future that night. She only decided that the past would no longer be edited for her convenience.`,
    `Before leaving, ${hero} took a picture of ${object}. Not for revenge. For the days when they would try to make her doubt herself again.`,
    `${hero} pushed her chair back slowly. "Tomorrow," she said, "we talk about what this cost. Tonight, nobody rewrites it."`,
    `The apology came late and imperfect. ${hero} accepted neither the excuse nor the performance, only the first clean fact.`,
    `${hero} left ${object} in the middle of the table, where everyone had to walk around it like a truth with weight.`
  ], `${seed}:choice`);
  return [pressure, dialogueTurn, evidence, reveal, escalation, choice];
}
...
function buildDiverseGeneratedStoryText({ profile, diversity, emotion, length, keywords, emotionGuidance = {} }) {
  ...
  const frameBeats = narrativeFlexibleBeats({
    ...
    twistDevice,
    peakEmotion,
    seed: `${diversity.frame_id}:${hero}:${object}:${turn}:${texture.title_template || ""}`
  });
  const paragraphs = [
    opening,
  ...
  const titleRelation = String(relation || "").replace(/^./, (letter) => letter.toUpperCase());
  const title = String(texture.title_template || "{hero} found {object}, and {relation} finally told the part everyone hid")
    ...
    .replaceAll("{relation}", titleRelation);
...
function latestPrepublishPreviews(limit = 20) {
  return [...readPrepublishPreviews()]
    .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0))
    .slice(0, limit);
}

function prepublishPreviewByRef(ref = "1") {
  const index = Number(ref);
  if (Number.isInteger(index) && index >= 1) return latestPrepublishPreviews(20)[index - 1] || null;
  return readPrepublishPreviews().find((item) => item.id === ref || item.package_id === ref) || null;
}

function prepublishFacebookPostText(draft = {}) {
  const optimized = optimizeFacebookPost({
    title: draft.title || "",
    facebook_text: draft.hook || "",
    website_text: draft.full_story || draft.moral || ""
  });
  const text = stripLinks(optimized.facebook_text || draft.hook || draft.title || "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
  return text || "Preview unavailable: draft text is missing.";
}
...
async function createPrepublishPreviewForPackage(ref = "1") {
  const readiness = await readinessGateCheckPackage(ref || "1");
  if (readiness.status !== "ready") {
    return {
      ok: false,
      module: "Pre-Publish Simulator v1",
      code: "package_not_ready",
      message: "Package must pass Final Readiness Gate before preview.",
      readiness,
      safety: { publishing_enabled: false, facebook_posting_enabled: false, write_permissions_required: false }
    };
  }
  const details = publishingPackageDetails(readiness.package);
  ...
  const preview = {
    ...
    readiness_score: Number(readiness.readiness_score || 0),
    expected_reaction: prepublishExpectedReaction({
      draft,
      editorialScore: Number(readiness.editorial?.final_editorial_score || 0),
      readinessScore: Number(readiness.readiness_score || 0),
      safetyScore: Number(readiness.safety_review?.safety_score || 0)
    }),
    risk_warnings_json: prepublishRiskWarnings(readiness, details),
...
function buildPrepublishDashboardData() {
  const previews = latestPrepublishPreviews(20);
  const previewedPackageIds = new Set(previews.map((item) => item.package_id));
  const readyWithoutPreview = readyPublishingPackages()
    .filter((pkg) => !previewedPackageIds.has(pkg.id))
    .map((pkg) => readinessGateCheckPackageSync(pkg));
  return {
    ok: true,
    module: "Pre-Publish Simulator v1",
    latest_previews: previews,
    ready_without_preview: readyWithoutPreview,
    preview_status: {
      previews_count: readPrepublishPreviews().length,
      ready_without_preview_count: readyWithoutPreview.length,
      publishing_enabled: false,
      facebook_write_permissions_required: false
    },
    safety: { publishing_enabled: false, facebook_posting_enabled: false, autopublishing: false }
  };
}
...
function renderAutopilotV1Dashboard() {
  ...
  return layout("AI Autopilot v1", `${renderHeader()}
    <main class="insights-page">
      ...
      <section class="insight-card">
        <h2>Pre-Publish Simulator v1</h2>
        <div class="autopilot-status-grid">
          ${card("Latest Previews", prepublish.preview_status.previews_count, "Exact post preview before any Facebook publishing exists")}
          ${card("Ready Without Preview", prepublish.preview_status.ready_without_preview_count, "Ready packages still need simulation")}
          ${card("Publishing", "disabled", "No Facebook write permissions or autopublishing")}
        </div>
        <div class="facebook-table-wrap">
          <table class="facebook-table">
...
async function telegramPrepublishPreview(chatId, numberText = "1") {
  const result = await createPrepublishPreviewForPackage(numberText || "1");
  return sendTelegramLongMessage(chatId, telegramPrepublishPreviewText(result), mainTelegramKeyboard());
}
...
async function handleTelegramMessage(message) {
  ...
  if (command === "/preview" || command === "/предпросмотр") return telegramPrepublishPreview(chatId, args[0] || "1");
  if (command === "/previews" || command === "/предпросмотры") return telegramPrepublishPreviews(chatId);
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === "/api/prepublish/v1/preview-package" && req.method === "POST") {
    const payload = await parseBody(req);
    return sendJson(res, 200, await createPrepublishPreviewForPackage(payload.package || payload.package_id || payload.index || "1"));
  }

  if (pathname === "/api/prepublish/v1/previews" && req.method === "GET") {
    return sendJson(res, 200, buildPrepublishDashboardData());
  }

  if (pathname.startsWith("/api/prepublish/v1/preview/") && req.method === "GET") {
    const ref = decodeURIComponent(pathname.replace("/api/prepublish/v1/preview/", ""));
    const preview = prepublishPreviewByRef(ref || "1");
    return sendJson(res, preview ? 200 : 404, preview
      ? {
          ok: true,
          module: "Pre-Publish Simulator v1",
          preview,
      ...
      : { ok: false, code: "preview_not_found" });
  }
